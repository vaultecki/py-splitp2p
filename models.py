...
class User:
    ...
    def canonical_bytes(self) -> bytes:
        return (
            f"{self.public_key}|{self.group_id}|{self.name}|{self.timestamp}|{self.lamport_clock}"
        ).encode()
    ...
    @classmethod
    def create(cls, public_key: str, name: str, group_id: str, lamport_clock: int = 0) -> "User":
        return cls(
...
def split_equally(
    expense_id: str, amount: int, payer_key: str, debtor_keys: list[str], lamport_clock: int = 0
) -> list[Split]:
...
def split_custom(
    expense_id: str, payer_key: str, amounts: dict[str, int], lamport_clock: int = 0
) -> list[Split]:
...
def split_by_percent(
    ...
    """Splits amount proportionally to percentages (normalized, need not sum to 100)."""
    total_pct = sum(percentages.values())
    if total_pct <= 0 or not percentages:
        return []
    keys = list(percentages.keys())
    shares = [round(amount * percentages[k] / total_pct) for k in keys]
    shares[0] += amount - sum(shares)  # remainder goes to first debtor
    return [
        Split.create(
            belongs_to=expense_id,
            payer_key=payer_key,
...
class Expense:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        return (
            f"{self.id}|{self.group_id}|{self.amount}"
...
class UserComment:
    ...
    def canonical_bytes(self) -> bytes:
        return (
            f"{self.id}|{self.belongs_to}|{self.comment or ''}"
            f"|{self.author_pubkey}|{self.timestamp}"
            f"|{self.lamport_clock}|{self.is_deleted}"
        ).encode()
    ...
    @classmethod
    def create(
        cls, belongs_to: str, comment: str, author_pubkey: str, lamport_clock: int = 0
    ) -> "UserComment":
        return cls(
            id=str(uuid.uuid4()),
            belongs_to=belongs_to,
...
class Attachment:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        return (
            f"{self.id}|{self.belongs_to}|{self.sha256}"
    ...
    def from_wire_dict(cls, d: dict) -> "Attachment":
        return cls(
            ...
            size=d.get("size"),
            signature=d.get("signature", ""),
            is_stored=ATTACHMENT_NOT_STORED,
        )  # always reset on receive
